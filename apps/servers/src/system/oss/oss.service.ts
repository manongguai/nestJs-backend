...
export class OssService {
  ...
  constructor(
    ...
    try {
      ...
      throw new Error(`文件存储路径配置 app.file.location = ${configLocation} (完整路径： ${this.basePath} ) 无写入权限`);
    }
  }

  async create(files: Express.Multer.File[], business: string, user: { id: string; username: string }): Promise<ResultData> {
